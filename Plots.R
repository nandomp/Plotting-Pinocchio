...
#Correctness
corr <- ggplot(sk, aes(logParams, Correctness)) +
  geom_smooth(aes(logParams, Correctness, colour = Type), se = FALSE, method = "lm") + 
...
#Avoidance
av <- ggplot(sk, aes(logParams, Avoidance)) +
  geom_smooth(aes(logParams, Avoidance, colour = Type), se = FALSE, method = "lm") + 
  geom_point(aes(colour = Type, shape = Family), size = 8, alpha = 1) + 
  geom_point(aes(shape = Family), size = 7.5, alpha = 0.7, colour = "white") +
  ggtitle("Avoidance") +
...
caut <- ggplot(sk, aes(logParams, caut)) +
  geom_smooth(aes(logParams, caut, colour = Type), se = FALSE, method = "lm") + 
  geom_point(aes(colour = Type, shape = Family), size = 8, alpha = 1) + 
  geom_point(aes(shape = Family), size = 7.5, alpha = 0.7, colour = "white") +
...
reck <- ggplot(sk, aes(logParams, reck)) +
  geom_smooth(aes(logParams, reck, colour = Type), se = FALSE, method = "lm") + 
  geom_point(aes(colour = Type, shape = Family), size = 8, alpha = 1) + 
  geom_point(aes(shape = Family), size = 7.5, alpha = 0.7, colour = "white") +
  ggtitle("Avoidance") +
  labs(title = paste0("<strong><span style='color:black; font-size:16pt;'>","Ultracrepidarianism ",
                      ...
                      "</span></strong>  <span style='color:","black","; font-size:14pt;'>","vs",
                      "</span> <span style='color:","#01497c","; font-size:14pt;'>","SHAPED UP","</span>)")) +
  ylab("Ultracrepidarianism (a/(a+i))")+
  xlab("log(#Params)") + 
  geom_label_repel(aes(colour = Type,label = Engine),
...
pfrq <- read.csv("./HumanStudy/prolific_frequencies.csv")
head(pfrq)
...
rcount <- read.csv("./HumanStudy/question_histogram.csv")
head(rcount)
